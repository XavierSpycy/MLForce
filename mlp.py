...
class Scaler(object):
  """Data scaling with MinMax Normalization or Z-score Normalization"""
  def __init__(self, method='MinMax'):
    """Parameter(s):
        method, corresponds to normalization, default:'MinMax' 
    """
    self.method = method
  ...
  def fit(self, X):
    ...
    Note: If method equals to MinMax,
          param1, corresponds to the minimum of the input,
          param2, corresponds to the range of the input, 
                  which should be the maximum minus the minimum of the input;
          If method equals to Standard,
          param1, corresponds to the mean of the input,
          param2, corresponds to the standard deviation of the input.
    """
    if self.method == "MinMax":
      self.param1 = np.min(X, axis=0)
      self.param2 = np.max(X, axis=0) - self.param1
    elif self.method == "Standard":
      self.param1 = np.mean(X, axis=0)
      self.param2 = np.std(X, axis=0)

  def transform(self, X):
    """Re-scale the input"""
    X_scaled = (X - self.param1) / self.param2
    return X_scaled
...
class Dataset(object):
  ...
  def normalize_(self):
    ...
    scaler = Scaler(self.norm_method)
